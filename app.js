...
  // Pre-calculate head's forward path to ensure the body does not cross it
  const headPathCells = new Set();
  const dir = DIRS[headDirKey];
  let px = head.x + dir.x;
  let py = head.y + dir.y;
  while (inBounds(px, py, size)) {
    headPathCells.add(`${px},${py}`);
    px += dir.x;
    py += dir.y;
  }
...
      return !used.has(key) && !grid[n.y][n.x] && !headPathCells.has(key);
    });
...
  // Store a serializable version for restart
  state.initialBoard = {
    size: board.size,
...
    // Reconstruct the board from initialBoard
    const board = {
