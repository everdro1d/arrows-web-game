const CONFIG = {
  ...
  CELL_SIZE: 52,
  EMPTY_DOT_RADIUS_RATIO: 0.12,
  ARROW_STROKE_RATIO: 0.38,
  RAY_WIDTH_RATIO: 0.08,
  BLOCKED_SHAKE_MS: 260,
  BLOCKED_FLASH_MS: 340,
  FIRE_SPEED_CELLS_PER_SEC: 8,
  DIFFICULTIES: {
...
function buildLengths(totalCells, arrowCount) {
  const lengths = Array(arrowCount).fill(2);
  let extra = totalCells - arrowCount * 2;
  while (extra > 0) {
    lengths[randInt(lengths.length)] += 1;
    extra -= 1;
  }
  return shuffle(lengths).sort((a, b) => b - a);
}

function cellRayClear(grid, size, head, dir) {
  let x = head.x + dir.x;
  let y = head.y + dir.y;
...
function getNeighbors(cell, size) {
  return DIR_KEYS.map((k) => ({ x: cell.x + DIRS[k].x, y: cell.y + DIRS[k].y })).filter((n) => inBounds(n.x, n.y, size));
}

function buildBodyPath(head, headDirKey, length, grid, size) {
  const opposite = { N: 'S', S: 'N', E: 'W', W: 'E' }[headDirKey];
  const back = { x: head.x + DIRS[opposite].x, y: head.y + DIRS[opposite].y };
  if (!inBounds(back.x, back.y, size) || grid[back.y][back.x]) {
    return null;
  }
  const path = [back];
  const used = new Set([keyForCell(head), keyForCell(back)]);
  ...
  function grow() {
    ...
    const options = shuffle(getNeighbors(tail, size)).filter((n) => {
      const key = keyForCell(n);
      return !used.has(key) && !grid[n.y][n.x];
    });

    for (const next of options) {
      path.push(next);
      used.add(keyForCell(next));
      if (grow()) {
        return true;
      }
      used.delete(keyForCell(next));
      path.pop();
    }
    return false;
  }

  return grow() ? path : null;
}

function tryPlaceArrow(length, grid, size, arrows, idRef) {
  const heads = [];
  ...
  for (const head of heads) {
    ...
    for (const dirKey of dirs) {
      ...
      if (!cellRayClear(grid, size, head, dir)) {
        continue;
      }
      const body = buildBodyPath(head, dirKey, length, grid, size);
      if (!body) {
        continue;
      }
      const arrow = {
...
function generateBoard(difficultyKey) {
  ...
  for (let attempt = 0; attempt < 160; attempt += 1) {
    ...
    const idRef = { value: 1 };
    const lengths = buildLengths(total, diff.arrowCount);
    let ok = true;

    for (const length of lengths) {
      if (!tryPlaceArrow(length, grid, size, arrows, idRef)) {
        ok = false;
        break;
      }
    }
    ...
    if (ok) {
      return { size, grid, arrows, nextArrowId: idRef.value };
    }
  }
...
function updateTimer(now) {
  const elapsed = state.completed ? state.endTime - state.startTime : now - state.startTime;
  timerEl.textContent = state.startTime ? formatDuration(Math.max(0, elapsed)) : '00:00.0';
}
...
function arrowColor(arrow, now) {
  ...
  if (arrow.queued || arrow.firing || state.hoverArrowId === arrow.id || state.activeArrowId === arrow.id) {
...
function isPathBlocked(arrow) {
  ...
  while (inBounds(x, y, state.gridSize)) {
    if (state.grid[y][x] && state.grid[y][x] !== arrow.id) {
      return true;
    }
    x += dir.x;
    y += dir.y;
...
function fireOrQueueArrow(arrowId) {
  const arrow = state.arrows.get(arrowId);
  if (!arrow || state.completed) {
    return;
  }
  if (state.currentAnimation) {
    if (!arrow.queued && !arrow.firing) {
      arrow.queued = true;
      state.queue.push(arrow.id);
    }
    return;
  }
...
function runQueue() {
  while (state.queue.length > 0) {
    ...
    const arrow = state.arrows.get(nextId);
    if (!arrow || arrow.firing) {
      continue;
    }
    if (isPathBlocked(arrow)) {
...
function checkWin(now) {
  ...
  if (state.arrows.size === 0 && !state.currentAnimation) {
...
function updateAnimation(dt, now) {
  ...
  const newHead = { x: head.x + dir.x * dt * CONFIG.FIRE_SPEED_CELLS_PER_SEC, y: head.y + dir.y * dt * CONFIG.FIRE_SPEED_CELLS_PER_SEC };
  ...
  if (offscreen) {
    const arrowId = anim.arrowId;
    state.currentAnimation = null;
    state.activeArrowId = null;
    state.arrows.delete(arrowId);
...
function drawArrowCells(cells, dirKey, color, offsetX = 0) {
  ...
  const dir = DIRS[dirKey];
  const tip = {
    x: head.x + dir.x * stroke * 0.85 + offsetX,
    y: head.y + dir.y * stroke * 0.85
  };
  const perp = { x: -dir.y, y: dir.x };
  const sideA = {
    x: head.x - dir.x * stroke * 0.18 + perp.x * stroke * 0.45 + offsetX,
    y: head.y - dir.y * stroke * 0.18 + perp.y * stroke * 0.45
  };
  const sideB = {
    x: head.x - dir.x * stroke * 0.18 - perp.x * stroke * 0.45 + offsetX,
    y: head.y - dir.y * stroke * 0.18 - perp.y * stroke * 0.45
...
function drawAnimatedArrow(anim, color) {
  ...
  const tip = { x: h.x + dir.x * stroke * 0.85, y: h.y + dir.y * stroke * 0.85 };
  const perp = { x: -dir.y, y: dir.x };
  const sideA = { x: h.x - dir.x * stroke * 0.18 + perp.x * stroke * 0.45, y: h.y - dir.y * stroke * 0.18 + perp.y * stroke * 0.45 };
  const sideB = { x: h.x - dir.x * stroke * 0.18 - perp.x * stroke * 0.45, y: h.y - dir.y * stroke * 0.18 - perp.y * stroke * 0.45 };
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.moveTo(tip.x, tip.y);
  ctx.lineTo(sideA.x, sideA.y);
...
function drawRays(now) {
  ...
  if (state.currentAnimation) {
    const anim = state.currentAnimation;
    const head = positionOnPath(anim.path, 0);
    const start = worldToScreen(head);
    const dir = DIRS[anim.headDir];
    const end = worldToScreen(rayEndFromHead(head, dir));
    ctx.strokeStyle = CONFIG.COLOR_ARROW_ACTIVE;
    ctx.beginPath();
    ctx.moveTo(start.x, start.y);
    ctx.lineTo(end.x, end.y);
    ctx.stroke();
...
function render(now) {
  ...
  for (const arrow of state.arrows.values()) {
    ...
    if (arrow.shakeUntil > now) {
      shake = Math.sin(now * 0.06) * 4;
    }
    drawArrowCells(arrow.cells, arrow.headDir, color, shake);
  }

  if (state.currentAnimation) {
    drawAnimatedArrow(state.currentAnimation, CONFIG.COLOR_ARROW_ACTIVE);
  }
}
...
function onMouseMove(e) {
  ...
  if (state.pointer.mouseDown) {
    ...
    if (moved && (state.pointer.downOnEmpty || state.pointer.dragging)) {
